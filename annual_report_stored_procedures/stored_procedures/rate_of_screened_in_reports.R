...
###############################
# Rate of Screened-In Reports #
###############################

screened_rate <- sqlQuery(con_test_annie, "SELECT SI.*
                          ,LC.old_region_cd
                          FROM rate_referrals_scrn_in AS SI
                          JOIN ref_lookup_county_region AS LC
                          ON SI.county_cd = LC.county_cd;")	

####################################
# aggregating data to region level #
####################################

screened_sums <- screened_rate %>% group_by(qry_type, date_type, start_date, entry_point, old_region_cd) %>%
...
for(h in 0:6){	
    ...
    for (i in 0:6) {
        dat <- ts_func(screened_sums, old_region_cd == d & entry_point == i, x = 8)
...
# sqlDrop(con_test_annie, sqtable = "test_annie.rate_referrals_scrn_in_ts")
sqlSave(con_test_annie, dat = dat2, tablename = "test_annie.rate_referrals_scrn_in_ts", rownames = FALSE)